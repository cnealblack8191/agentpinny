...
def train_detector(dataset: str | os.PathLike, models_dir: str | os.PathLike | None = None,
    ...
    if save:
        from pinny.models.artifact import default_models_dir, read_metadata, save_artifact

        model_dir = save_artifact(models_dir or default_models_dir(), kind=fields["kind"],
                                  arch=fields["arch"], state_dict=fields["state_dict"],
                                  metadata={k: v for k, v in fields.items()
                                            if k not in ("kind", "arch", "state_dict")})
