...
def _save(net, tmp_path, threshold=0.4, created_at=None):
    ...
    return save_artifact(tmp_path / "models", kind="detector", arch=ARCH, state_dict=net.state_dict(),
                         metadata={"input": {"channels": 1, "tile_px": 512, "stride_px": 384, "dpi": 200},
...
def test_load_refuses_corrupt_weights_wrong_kind_and_overwrite(tmp_path):
    ...
    from pinny.models.artifact import load_artifact
    ...
    with pytest.raises(PinnyError) as e:
        load_artifact(model_dir, kind="verifier")
