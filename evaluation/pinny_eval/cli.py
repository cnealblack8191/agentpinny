...
def _adapt_scan(args: argparse.Namespace) -> int:
    ...
    print(
        ...
        f"page {loaded.identity.page_index}, raster {loaded.frame.width}x{loaded.frame.height} "
        + (f"at {loaded.frame.dpi:g} DPI" if loaded.frame.dpi is not None else "(dpi not declared)")
    )
    return 0
